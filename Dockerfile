...
COPY code-do-application/src /app/code-do-application/src
...
RUN mvn -s -f /app/pom.xml clean package
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/code-do-application-0.0.1-SNAPSHOT.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/code-do-application-0.0.1-SNAPSHOT.jar"]
